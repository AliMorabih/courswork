...
public class CityPopulationDAL {
    public ArrayList<CityPopulation> getWorldCityListByPop(Connection con)
    {
        try
        {
            ...
            String strSelect =
                    "SELECT id, city.name, country.name,country.continent,country.region, city.Population "
                            + "FROM country, city "
    ...
    {
        // Print header
        System.out.println(String.format("%-30s %-30s %-20s %-30s %-20s ", "CityName", "Country Name", "Continent" , "Region", "Population"));
        // Loop over all employees in the list
        for (CityPopulation city : citypopulation)
        {
            String emp_string =
                    String.format("%-30s %-30s %-20s %-30s %-20s",
                            city.CityName, city.CountryName, city.Continent,  city.Region, city.Population);
